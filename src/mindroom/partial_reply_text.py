...
_STREAM_ERROR_RESPONSE_NOTE_PREFIX = "**[Response interrupted by an error"
...
def format_stream_error_note(error: Exception) -> str:
    ...
    if not normalized_error:
        return f"{_STREAM_ERROR_RESPONSE_NOTE_PREFIX}. Please retry.]**"
    if len(normalized_error) > 220:
        normalized_error = f"{normalized_error[:219]}…"
    return f"{_STREAM_ERROR_RESPONSE_NOTE_PREFIX}: {normalized_error}]**"
...
def is_interrupted_partial_reply(text: object) -> bool:
    ...
    ) or (_STREAM_ERROR_RESPONSE_NOTE_PREFIX in trimmed_text)
...
def clean_partial_reply_text(text: str) -> str:
    ...
    if _STREAM_ERROR_RESPONSE_NOTE_PREFIX in cleaned:
        cleaned = cleaned.split(_STREAM_ERROR_RESPONSE_NOTE_PREFIX, 1)[0].rstrip()

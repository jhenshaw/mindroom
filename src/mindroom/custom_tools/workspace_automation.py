...
from mindroom.workspace_automations.targets import iter_workspace_automation_targets
...
class WorkspaceAutomationTools(Toolkit):
    ...
    async def list_automations(self) -> str:
        ...
        return custom_tool_payload(
            _TOOL_NAME,
            "ok",
            automations=[_loaded_status_payload(status) for status in service.list_loaded()],
        )
    ...
    async def reload_automations(self) -> str:
        ...
        try:
            scan_result = await service.scan_now()
